...
export default function PublicEventPage() {
  ...
  useEffect(() => {
    async function fetchEventData() {
      if (!eventId || eventId.startsWith("evt-") || eventId === "evt_123") {
        setLoading(false);
        // Default select first mock ticket
        if (mockTicketTypes.length > 0) {
          setSelectedTicketId(mockTicketTypes[0].id);
        }
      ...
      try {
        ...
        if (formFieldsData) {
          // Exclude default fields (FullName, Email) to prevent duplication in custom layout
  ...
  // Fallback to mock data if no live event was found (e.g. for developer environment demo links)
  ...
  // Generate a reliable Google Maps embed URL using the venue name or link query
  const getEmbedUrl = () => {
  ...
  const handleCheckoutSubmit = async (e: React.FormEvent) => {
    ...
    if (isMock) {
      // Mock flow success simulation
      setTimeout(() => {
        setSubmittingRegistration(false);
        setIsCheckoutOpen(false);
        router.push(`/tickets/guest-1`);
      }, 1000);
      return;
    }
      ...
      <div className="max-w-5xl mx-auto px-6 -mt-32 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            {/* Speakers */}
            {(event.show_speakers !== false && (eventId.startsWith("evt-") || eventId === "evt_123" ? mockSpeakers : liveSpeakers).length > 0) && (
              <div className="space-y-6 animate-slide-up" style={{ animationDelay: "200ms" }}>
                <h2 className="text-2xl font-bold text-slate-900">Speakers</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-5">
                  {(eventId.startsWith("evt-") || eventId === "evt_123" ? mockSpeakers : liveSpeakers).map((speaker) => (
            ...
            {/* Sponsors */}
            {(eventId.startsWith("evt-") || eventId === "evt_123" ? mockSponsors : liveSponsors).length > 0 && (
              <div className="space-y-6 animate-slide-up" style={{ animationDelay: "300ms" }}>
                ...
                <div className="flex flex-wrap items-center gap-8 md:gap-12 mt-8">
                  {(eventId.startsWith("evt-") || eventId === "evt_123" ? mockSponsors : liveSponsors).map((sponsor) => (
                    <a 
                      key={sponsor.id} 
